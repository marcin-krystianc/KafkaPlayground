...
public static class ProducerStringTask
{
    ...
    public static Task GetTask(ProducerConsumerSettings settings, ProducerConsumerData data, int producerIndex)
    {
        return Task.Run(async () =>
        {
            ...
            for (;; )
            {
                var currentValue = string.Join("", rng.GetItems(new []{'a', 'b', 'c'}, 10000));
                for (var topicIndex = 0; topicIndex < topicsPerProducer; topicIndex++)
                {
                    for (var partition = 0; partition < settings.Partitions; partition++)
                    {
                        var topicPartition =
                            topicPartitions[
                                (topicIndex + producerIndex * topicsPerProducer) * settings.Partitions + partition];
                        if (e != null)
                        {
                            throw e;
                        }
                        ...
                        var msg = new Message<int, string>
                        {
                            Key = partition, Value = currentValue, Timestamp = new Timestamp(DateTime.UtcNow)
                        };
                            ...
                            try
                            {
                                producer.Produce(topicPartition, msg,
                                    (deliveryReport) =>
                                    {
                                        ...
                                        {
                                            var latency = DateTime.UtcNow -
                                                          deliveryReport.Message.Timestamp.UtcDateTime;
